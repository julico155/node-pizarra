...
io.on("connection", (socket) => {
  ...
  socket.on("login", async ({ name, room }) => {
    ...
    const responseProject = await axios.get(
      "http://18.223.2.7/api/cargar-diagrama/" + room
    );
  ...
  socket.on("save_component", async (data) => {
    if (findRoom(data.room)) {
      ...
      const response = await axios.put(
        "http://18.223.2.7/api/guardar-diagrama/" + data.room,
        body
      );
    }
  });
...
server.listen(8081);
console.log("servidor socket conectado en el puerto 8081");
module.exports = {
  sockets,
  server,
  io,
